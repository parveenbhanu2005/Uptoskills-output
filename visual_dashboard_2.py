...
def draw_dashboard(frame, status, color, risk, frame_count):
    ...
    cv2.putText(frame, "TARGET: PUMP_042 | SENSOR: ACOUSTIC_MIC_04", (30, 70), cv2.FONT_HERSHEY_SIMPLEX, 0.5, GREEN, 1)
    ...
    for x in range(40, 520, 12):
        ...
        elif status == "WARNING STATE":
            noise = np.random.randint(30, 70)
        else:
            noise = np.random.randint(60, 160)
            
        cv2.line(frame, (x, center_y - noise), (x, center_y + noise), color, 2)
        
        if status == "ANOMALY DETECTED" and noise > 120:
...
for i in range(300): 
    if i < 100:
        ...
        current_color = GREEN
        current_risk = np.random.randint(1, 6) 
    elif i < 180:
        current_status = "WARNING STATE"
        current_color = YELLOW
        current_risk = np.random.randint(45, 60) 
    else:
        current_status = "ANOMALY DETECTED"
        current_color = RED
        current_risk = 89 + np.random.randint(-2, 3) 
        
    final_ui = draw_dashboard(dashboard_frame, current_status, current_color, current_risk, i)
    cv2.imshow("Acoustic Monitoring Feed - Pump 042", final_ui)
